...
def init_logging_handler(folder_name):
...
def edges_to_sparse(edges, N, values=None):
    ...
    return scipy.sparse.coo_matrix(
        (values, (edges[:, 0], edges[:, 1])), shape=(N, N)
    ).tocsr()
...
def train_val_test_split_adjacency(
    ...
    if undirected:
        ...
        A = scipy.sparse.tril(A).tocsr()  # consider only upper triangular
        A.eliminate_zeros()
    else:
        if is_undirected:
            warnings.warn(
    ...
    if every_node:
        if connected:
            assert (
                scipy.sparse.csgraph.connected_components(A)[0] == 1
            )  # make sure original graph is connected
            A_hold = scipy.sparse.csgraph.minimum_spanning_tree(A)
        else:
    ...
    if asserts:
        ...
        assert (
            not connected or scipy.sparse.csgraph.connected_components(A_hold)[0] == 1
        )
...
def ScipySparse2TorchSparse(matrix):
    """
    :param data: a scipy sparse csr matrix
    :return: a sparse torch tensor
    """
    coo_matrix = matrix.tocoo()
    indices = torch.LongTensor(np.array([coo_matrix.row, coo_matrix.col]))
    return torch.sparse_coo_tensor(
        indices=indices, values=torch.from_numpy(matrix.data).float(), size=matrix.shape
    )


def sample_zeros(A, n):
    def sample_zero(A):
        nonzero_or_sampled = set(zip(*A.nonzero()))
        while True:
            edge = tuple(np.random.randint(0, A.shape[0], size=2))
            if edge not in nonzero_or_sampled:
                yield edge
                nonzero_or_sampled.add(edge)

    itr = sample_zero(A)
    val_zeros = [next(itr) for _ in range(n)]
...
def sample_ones(A, n):
    ...
    val_ones = random.sample(val_ones, n)
...
def score_node_classification(features, z, p_labeled=0.1, n_repeat=10, norm=False):
    ...
    for seed in range(n_repeat):
        sss = StratifiedShuffleSplit(
            n_splits=1, test_size=1 - p_labeled, random_state=seed
        )
    ...
    return np.vstack(
        (
            ...
            np.array(
                [
                    [
                        -1 if len(x) == 0 else np.random.choice(x)
                        for x in hops[h].rows[nodes]
                    ]
                    for h in hops.keys()
                    if h != -1
                ]
...
def to_triplets(sampled_hops, scale_terms):
    ...
    for i, j in itertools.combinations(np.arange(1, sampled_hops.shape[1]), 2):
        ...
        triplet_scale_terms.append(
            scale_terms[i][triplet[:, 1]] * scale_terms[j][triplet[:, 2]]
        )
...
def get_row_MRR(probs, true_classes):
    ...
    existing_ranks = np.arange(1, true_classes.shape[0] + 1, dtype=np.float64)[
...
def get_MRR(predictions, true_classes, adj):
    ...
    pred_matrix = scipy.sparse.coo_matrix((probs, (adj[0], adj[1]))).toarray()
    true_matrix = scipy.sparse.coo_matrix((true_classes, (adj[0], adj[1]))).toarray()
    row_MRRs = []
    ...
    avg_MRR = torch.tensor(row_MRRs).mean().numpy()
    return avg_MRR


def get_MAP_e(predictions, true_classes):
    probs = torch.sigmoid(predictions)
    probs = probs.detach().cpu().numpy()
